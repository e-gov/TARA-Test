...
class SidW2aCancelEndpointSpec extends TaraSpecification {

    def setup() {
        flow.cookieFilter = new CookieFilter()
    }

    def "Cancel Smart-ID web2app polling with invalid session cookie: #reason"() {
        given:
        ...
        Response response = given()
                ...
                .post(flow.loginService.sidWeb2AppPollCancelUrl)
    ...
    def "Cancel Smart-ID web2app polling with invalid csrf token: #reason"() {
        given:
        Steps.startAuthenticationInTara(flow, "openid smartid")
        SidSteps.initSidWeb2AppAuthSession(flow)
    ...
    def "Cancel Smart-ID web2app polling with invalid method #requestType"() {
        given:
        Steps.startAuthenticationInTara(flow, "openid smartid")
        SidSteps.initSidWeb2AppAuthSession(flow)
        ...
        Response response = given()
                .cookies(["__Host-SESSION": flow.sessionId])
                .params([_csrf: flow.csrf])
                .request(requestType, flow.loginService.sidWeb2AppPollCancelUrl)
